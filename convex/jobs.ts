...
export const nightlyFederalSync = internalAction({
  args: {
    ...
    // maxPages = number of SAM.gov API requests (10 req/day free limit).
    // Default 2: conservative daily increment. Use up to 8 for manual backfills.
    maxPages: v.optional(v.number()),
  },
  returns: v.null(),
  handler: async (ctx, args) => {
    console.log("Starting SAM.gov sync...");
    const result = await ctx.runAction(internal.actions.syncSamGov.syncSamGov, {
      daysBack: args.daysBack ?? 1,
      maxPages: args.maxPages ?? 2,
    });
    console.log(
      `SAM.gov sync complete: ${result.requestsUsed} API requests used, ` +
        `${result.total} fetched, ${result.newOpportunities} new` +
        (result.rateLimited ? " ⚠️ RATE LIMITED — try again tomorrow" : ""),
    );
    return null;
  },
});
